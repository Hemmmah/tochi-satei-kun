...
1. ユーザーの土地査定依頼を受け取る
2. 必要入力を確認する
3. 不足している必須情報は勝手に補わず確認する
4. canonical Python査定処理（`scripts/main.py`）を呼び出す
5. 中間結果と警告を確認する
6. Excel／JSONを生成する
7. 人が確認すべき事項を返す
...
- **v1.5.0 (2026-08-09)**: Codex と Claude Code の共通Agent Skill化。canonical Skillを `skills/tochi-satei-kun/` に固定し、Codex package と Claude package を `scripts/sync_agent_plugins.py` で同期する構成を追加。査定ロジック・査定値は変更なし。
